...
namespace X86_64_EMU_SOFT::SYSTEM::CPU {
	template<INSTRUCTIONS::InstructionType Type>
	inline DEFINE_HANDLER(Handler_rm8_r8) {
		ZoneScoped;//NOLINT
		...
		const auto sourceFlags = static_cast<uint8_t>(std::to_underlying(INSTRUCTIONS::OPERANDS::RegisterOperandFlags::isGeneralPurposeRegister) |
	...
	template <INSTRUCTIONS::InstructionType Type>
	inline DEFINE_HANDLER(Handler_rm16rm32rm64_r16r32r64) {
		ZoneScoped;//NOLINT
		instruction.Type = Type;
		instruction.OpcodeBytes[0] = byte;
		instruction.OpcodeSizeBytes++;
	...
	template <INSTRUCTIONS::InstructionType Type>
	inline DEFINE_HANDLER(Handle_r8_rm8) {
		ZoneScoped;//NOLINT
		instruction.Type = Type;
		instruction.OpcodeBytes[0] = byte;
		instruction.OpcodeSizeBytes++;
	...
	template <INSTRUCTIONS::InstructionType Type>
	inline DEFINE_HANDLER(Handle_r16r32r64_rm16rm32rm64) {
		ZoneScoped;//NOLINT
		instruction.Type = Type;
		instruction.OpcodeBytes[0] = byte;
		instruction.OpcodeSizeBytes++;
	...
	template <INSTRUCTIONS::InstructionType Type>
	inline DEFINE_HANDLER(Handle_AL_imm8)noexcept {
		ZoneScoped;//NOLINT
		std::ignore = prefixes;
		instruction.Type = Type;
		instruction.OpcodeBytes[0] = byte;
		instruction.OpcodeSizeBytes++;
		instruction.InstructionLengthBytes++;
		constexpr auto destinationRegister = INSTRUCTIONS::TargetRegister::RAX;
		constexpr uint8_t destinationFlags = std::to_underlying(INSTRUCTIONS::OPERANDS::RegisterOperandFlags::isGeneralPurposeRegister);
		const auto destinationOperand = INSTRUCTIONS::OPERANDS::RegisterOperand{
			.RegisterPointer = std::bit_cast<std::array<uint8_t, 8>>(&core.GetRegister(destinationRegister)),
			.SizeBits = 8,
			.Flags = destinationFlags
	...
	template <INSTRUCTIONS::InstructionType Type>
	inline DEFINE_HANDLER(Handle_AxEaxRax_imm16imm32)noexcept {
		ZoneScoped;//NOLINT
		std::ignore = prefixes;
		instruction.Type = Type;
		instruction.OpcodeBytes[0] = byte;
		...
		constexpr auto destinationFlags = static_cast<uint8_t>(std::to_underlying(INSTRUCTIONS::OPERANDS::RegisterOperandFlags::isGeneralPurposeRegister) |
